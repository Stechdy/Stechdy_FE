...
const SubjectDetail = () => {
  ...
  const fetchSubjectData = async () => {
    try {
      ...
      // Fetch subject info
      const subjectResponse = await fetch(`http://localhost:3001/api/subjects/${id}`, {
      ...
      const sessionsResponse = await fetch(`http://localhost:3001/api/study-sessions/subject/${id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      const deadlinesResponse = await fetch(`http://localhost:3001/api/deadlines/subject/${id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const calculateProgress = () => {
    ...
    const completedMinutes = sessions
      ...
      .reduce((total, session) => {
        const duration = session.actualDuration || getDurationMinutes(session.startTime, session.endTime);
        ...
        <main className="subject-main">
          ...
          <div className="subject-progress-card">
            ...
            <div className="progress-hours">
              <span className="hours-text">{progress.completed}h / {progress.total}h</span>
            </div>
          </div>

          {/* Weekly Schedule */}
          <section className="weekly-schedule-section">
            <h2 className="subject-section-title">Weekly Schedule</h2>
            <div className="schedule-list">
              {weeklySessions.length === 0 ? (
                <div className="subject-no-sessions">No sessions scheduled this week</div>
              ) : (
                weeklySessions.map((session, idx) => (
                  ...
                  >
                    ...
                    <div className="schedule-info">
                      ...
                      <span className="schedule-duration">{formatDuration(session.startTime, session.endTime)}</span>
                    </div>
                    <span 
          ...
          <section className="deadlines-section">
            <div className="section-header">
              <h2 className="subject-section-title">Upcoming Deadlines</h2>
              <button className="view-all-btn" onClick={() => {}}>View All</button>
            </div>
            <div className="deadlines-list">
              {upcomingDeadlines.length === 0 ? (
                <div className="subject-no-deadlines">No upcoming deadlines</div>
              ) : (
                upcomingDeadlines.slice(0, 5).map((deadline, idx) => (
                  <div key={idx} className="deadline-item">
                    <div 
                      className="deadline-indicator"
                      style={{ backgroundColor: getPriorityColor(deadline.priorityLevel) }}
                    ></div>
                    <div className="deadline-info">
                      <h4 className="deadline-title">{deadline.title}</h4>
                      <span className="deadline-due">
                        <span className="due-icon">
                          {getDaysUntilDue(deadline.dueDate).includes('Overdue') ? '❗' : '🕐'}
                        </span>
                    ...
                    <span 
                      ...
                      style={{ 
                        ...
                        backgroundColor: `${getPriorityColor(deadline.priorityLevel)}20`
                      }}
                    >
                      {deadline.priorityLevel.charAt(0).toUpperCase() + deadline.priorityLevel.slice(1)}
                    </span>
                  </div>
                ))
              )}
              ...
              <div className="modal-body">
                ...
                <div className="form-group">
                  ...
                  <select
                    ...
                    onChange={(e) => setNewDeadline({...newDeadline, deadlineType: e.target.value})}
                  >
                ...
                <div className="form-group">
                  ...
                  <select
                    ...
                    onChange={(e) => setNewDeadline({...newDeadline, priorityLevel: e.target.value})}
                  >
